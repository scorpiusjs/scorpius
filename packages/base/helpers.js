 ...
 */
scorpius.helpers.searchObjectWithDots = function(object, key, selectFirstIfIsArray) {
  key = key.split('.');
 ...
 * Deep extend
 */
scorpius.helpers.deepExtend = function(target, source) {
  for (var prop in source)
    if (prop in target && typeof(target[prop]) == 'object' && typeof(source[prop]) == 'object')
      scorpius.helpers.deepExtend(target[prop], source[prop]);
    else
 ...
 * Useful for autoform
 */
scorpius.helpers.getTranslation = function(key) {
  return function() {
    return i18n(key);
  };
};
